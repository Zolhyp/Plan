...
header("Content-Type: text/html;charset=utf-8");
 //if(isset($_POST['agregar']) { 	
	...
	$fechakm="'$fechakm'";
	$poliza= $_POST['poliza'];
	$query="SELECT * FROM vehiculo where  noseguro = '".$poliza."' and id!=".$idento."";
	$aber= mysqli_query($conexion, $query);
	$row_cnt = $aber->num_rows;
	...
	$obser = $_POST["observ"] ;	
	if($error==0){
	$insertar = "UPDATE vehiculo SET placa='$placa', tipo='$tipo', marca='$marca', submarca='$submarca', modelo=$modelo, km=$km, fechakm=$fechakm, noseguro=$poliza, nombseguro='$aseguradora', fechaseguro='$fechaseg', observaciones='$obser', estado='$estado', nprovisional='$numerop', idCorralon='$corralon', noeconomico='$noeconomico' WHERE id=$idento";
