<?php
	...
	$salida="";
	$query="select id, tel, nombre, app, apm, municipio from corralon";
	if(isset($_POST['consulta'])){
		$q = $mysqli->real_escape_string($_POST['consulta']);
		$query ="select id, tel, nombre, app, apm, municipio from corralon where id like '%$q%'";
	}
	$resultado = $mysqli->query($query);
	if($resultado->num_rows > 0){
		$salida = "<br><label class='input-48'>Aseguradoras</label><br>
                		<div class='form-group'>  
                     		<form name='formulario' id='formulario' action = 'ase.php' type='submin' method = 'post' >  
		...
		while($fila = $resultado->fetch_assoc()){
			$salida.="<tr name='id' value='".$i."'>
                           ...
                           <td><input type='text' name = 'encargado[]' class='form-control name_list' style='width: 100%;' value = ".$fila['nombre']." ".$fila['app']." ".$fila['apm']." readonly='readonly'/></td>
                           <td><input type='text' name = 'municipio[]' class='form-control name_list' style='width: 100%;' value = ".$fila['municipio']." readonly='readonly'/></td>
