<?php
	...
	if($opcion[1] == 0){
		...
		while($fila = $resultado->fetch_assoc()){
			...
			$noseguro=utf8_encode($fila['poliza']);
			$nombseguro=utf8_encode($fila['rfcseguro']);
    ...
    $mysqli2 = new mysqli("localhost","root","root","SRB");
    $query2="select nombre, rfc from aseguradora where rfc = '".$nombseguro."'";
    $resultado2 = $mysqli2->query($query2);
    while($fila2 = $resultado2->fetch_assoc()){
      $aseguradora2=utf8_encode($fila2['nombre']);
    }
		echo "
<!DOCTYPE html>
<html lang = 'es'>
...
echo "<span id =b>";
        ...
        <option value = '".$nombseguro."'>".$aseguradora2."</option>
        ";
            ...
            if($tipo!="Carrito"&&$tipo!="Carreta"){
                ...
                <div class='form-group'>  
                     <form name='add_name' id='add_name'>  
                          <div class='table-responsive'>  
                            ...
                            $query="SELECT * FROM contenedor WHERE vehiculo_id = '".$identifica."'";
               ...
               <center>   
<a href='ConsultaVehiculos.php'><input type='button' value='Regresar' class='btn-agregar'></a>
</center>
               </div></p></div>
          </section>
		</form>
 ...
 //if(isset($_POST['agregar']) { 	
			...
			$insertar = "DELETE from contenedor where vehiculo_id = '".$id[$indice]."'";
...
echo "<span id =b>";
        ...
        <select class='input-48' name='aseguradora' id='aseguradora' required/>";
                         $conexion = new mysqli("localhost","root","root", "SRB") or die("No se pudo establecer conexion");
                            $query="SELECT nombre, rfc FROM aseguradora";
                            $resultado=$conexion->query($query);
                            if (!$resultado) {
                                echo 'No se pudo ejecutar la consulta: ' . mysql_error();
                                exit;
                              }
                              echo "<option value = '".$nombseguro."'>".$aseguradora2."</option>";
                            while ($row = $resultado->fetch_assoc()) {
                              if(utf8_decode($row['rfc'])!=$nombseguro){
