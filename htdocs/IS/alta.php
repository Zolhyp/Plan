...
header("Content-Type: text/html;charset=utf-8");
 //if(isset($_POST['agregar']) { 	
	...
	$fechakm="'$fechakm'";
	$poliza= $_POST['poliza'];
	$query="SELECT * FROM vehiculo where  noseguro = '".$poliza."'";
	$aber= mysqli_query($conexion, $query);
	$row_cnt = $aber->num_rows;
	...
	$obser = $_POST["observ"] ;	
	if($error==0){
	$insertar = "INSERT INTO vehiculo(placa, tipo, marca, submarca, modelo, km, fechakm, noseguro, nombseguro, fechaseguro, observaciones, estado, nprovisional, idCorralon, noeconomico)
    ...
    $query="SELECT id FROM vehiculo WHERE noseguro = ".$poliza."";
		...
		if($number > 0) {   
			for($i=0; $i<$number; $i++){  
				if(trim($_POST["name"][$i] != '')){  
					$resultado = "INSERT INTO contenedor VALUES('".mysqli_real_escape_string($conexion, $_POST["name"][$i])."', ".mysqli_real_escape_string($conexion, $_POST["name2"][$i]).", '".mysqli_real_escape_string($conexion, $_POST["fpeso"][$i])."', $qwery, $i)"; 					
					#echo "$resultado";
	...
	if(!$aber){
		...
		header('Refresh: 0.1; URL=altasp.php');
			...
			header('Refresh: 0.1; URL=altasE.html');
